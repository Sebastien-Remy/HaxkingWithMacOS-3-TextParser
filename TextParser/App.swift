...
struct App: ParsableCommand {
    ...
    static var configuration: CommandConfiguration {
        CommandConfiguration(commandName: "analyse", abstract: "Analyse input text using a range of natural language approaches.")
    }
    ...
    @Option(name: .shortAndLong, help: "The maximum number of alternative to suggest")
    var maximumAlternative = 10
     

    mutating func run()  {
        if detectLanguage == false && sentimentAnalysis == false && lemmatize == false && alternatives == false && names == false {
            
            // No option means all options
            detectLanguage = true
        ...
        if names {
            ...
            print()
            print ("Found the folowwing entities:")
    ...
    func embeddings(for word: String) -> [String] {
        ...
        if let embedding = NLEmbedding.wordEmbedding(for: .english) {
            let similardWords = embedding.neighbors(for: word, maximumCount: maximumAlternative)
    ...
    func entities(for string: String) -> [String] {
        ...
        tagger.enumerateTags(in: string.startIndex..<string.endIndex, unit: .word, scheme: .nameType, options: .joinNames) {
            tag, range in
